...
export default function TicketsQuickDock({
  serviceColor,
  onNew,
  onUrgent,
  onSearch,
  ...
  const actions = [
    ...
    { label: "Urgenti", icon: WarningAmberIcon, onClick: onUrgent, color: "#ef4444" },
  ...
  return (
    ...
    >
      ...
      <Box component={motion.div} whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.96 }}>
        <Tooltip title="Nuovo ticket" placement="left">
          <Fab
            onClick={onNew}
            sx={{
              background: `linear-gradient(135deg, ${serviceColor} 0%, #06b6d4 100%)`,
              color: "#fff",
